...
def tela_login(supabase):
    ...
    if submit:
        try:
            ...
            cookie_manager.set("sb_access_token", res.session.access_token, expires_at=expire_date, key="set_access", secure=True)
            cookie_manager.set("sb_refresh_token", res.session.refresh_token, expires_at=expire_date, key="set_refresh", secure=True)
