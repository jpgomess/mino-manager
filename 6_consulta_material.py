...
# Pegamos os nomes únicos e ordenamos
lista_materiais = sorted(df["Descrição"].unique())
material_selecionado = st.selectbox("Selecione o Material para investigar:", lista_materiais)

# 4. Filtrar dados apenas desse material
df_filtrado = df[df["Descrição"] == material_selecionado].copy()
...
with tab1:
    st.subheader(f"Todas as compras de '{material_selecionado}'")
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Valor": st.column_config.NumberColumn("Preço Unit.", format="R$ %.2f"),
...
with tab2:
    ...
    fig_qtd = px.bar(
        ...
        x="Obra", 
        y="Quantidade", 
        title=f"Consumo de '{material_selecionado}' por Obra (Qtd)",
        text_auto=True
    )
    ...
    if "Data" in df_filtrado.columns:
        fig_preco = px.scatter(
            ...
            color="Obra",
            size="Quantidade",
            title=f"Histórico de Preço Unitário: '{material_selecionado}'",
